...
python ../pyseer-runner.py --kmers kmers.gz --phenotypes subset.cont.pheno --max-dimensions 3 --load-m pop_struct.pkl > 5.log 2> 5.err || die "Continuous phenotype"
...
python ../pyseer-runner.py kmers.txt --phenotypes subset.pheno --load-m pop_struct.pkl > /dev/null 2> /dev/null && die "Uncompressed kmers but no option"
...
python ../pyseer-runner.py --kmers kmers.gz --phenotypes supersubset.cont.pheno --load-m pop_struct.pkl > /dev/null 2> /dev/null || die "Weak results for continuous phenotype"
